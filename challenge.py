...
# Create a function to take in three arguments 
def etl_format(wiki_movies_raw, kaggle_metadata, ratings):
    return
...
# Drop duplicate rows via imdb id
...
# Dropping Nan from 'Box Office' column
...
#Use regex to create an expression
...
# Assuming that there are will be missing values in columns and those columns will need to be dropped. 
# And that the data type of the budget column will need to be replaced 
try:
...
# Assumption is that the data in the 'Release date' column is some type of date-time data
# If not in date-time type, drop column
try:
...
# Asumption same as 'Release date,' "Running time" data is some type of time-data-type
# If not, drop column
try:
...
# Join wiki_movies_df to kaggle_metadata
